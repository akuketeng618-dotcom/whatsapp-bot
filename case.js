...
module.exports = sock = async (sock, m, msg, chatUpdate, store) => {
	try {
					...
					if (regex.test(budy)) {
						...
						if (db.data.users[m.sender].badword > 5) {
							try {
								...
								await sock.sendMessage(m.chat, { text: `⚠️ Ups, aku gagal kick pengguna ini. Pastikan bot memiliki hak admin!` });
							}
						} else {
		...
		if (autoJoinNewsletter) {
						await sock.newsletterFollow('120363323868732166@newsletter');
						await sleep(3000);
          	await sock.newsletterFollow('120363418977603376@newsletter');
          	await sleep(3000);
          	await sock.newsletterFollow('120363403600616189@newsletter');
		};
